...
class Reservation
{
    ...
    public function getAll($user_ref)
    {
        $this->db->query('SELECT reservations.* , halls.name AS "hall_name" FROM reservations INNER JOIN halls ON reservations.hall_id = halls.id AND user_ref = :user_ref');
    ...
    {
        $this->db->query('INSERT INTO reservations (hall_id , user_ref , seat_number , showed_at) VALUES (:hall_id , :user_ref , :seat_number , :showed_at)');
        $this->db->bind(":hall_id", $data['hall_id']);
        $this->db->bind(":user_ref", $data['user_ref']);
        $this->db->bind(":seat_number", $data['seat_number']);
        $this->db->bind(":showed_at", $data['showed_at']);
    ...
    {
        $this->db->query('SELECT * FROM reservations WHERE DATEDIFF(showed_at , CURDATE()) > 1 AND id = :res_id');
